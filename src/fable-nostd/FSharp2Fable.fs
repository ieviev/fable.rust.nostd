...
let private transformBaseConsCall
    ...
    match
        Replacements.Api.tryBaseConstructor com ctx baseEntRef argTypes genArgs baseArgs
    with
    | Some(baseRef, args) ->
        let callInfo =
    ...
    trampoline {
        ...
        let expr =
            ...
            | Some(_, Some _fullName) ->
                match expr with
                | Fable.CurriedApply(expr2, [ Fable.Value(Fable.UnitConstant, _) ], _, _) ->
                    expr2
                | Fable.Call(expr2,
                             {
...
let private transformUnionCaseTest
    ...
    trampoline {
        ...
        | ErasedUnion(tdef, genArgs, rule) ->
            ...
            | 0 ->
                return
                    makeEqOp r unionExpr (transformStringEnum rule unionCase) BinaryEqual
            | 1 ->
...
// [<return: Struct>]
// let (|HasReplacement|_|)(e: FSharpExpr) =
//     match e with
//     | FSharpExprPatterns.Call(objExprOpt, memberOrFunc, typeArgs1, typearg, argExprs) ->
//         stdout.WriteLine $"{memberOrFunc.FullName} : {memberOrFunc.CompiledName}"

//         match memberOrFunc.FullName with
//         | "Fable.Rust.Nostd.str" ->
//             ()
//             // let e = makeStrConst "replaced"
//             // ValueSome e
//             ValueNone
//         | _ -> ValueNone
...
let private transformExpr (com: IFableCompiler) (ctx: Context) appliedGenArgs fsExpr =
    trampoline {
        // stdout.WriteLine $"expr: {fsExpr}"

        match fsExpr with
        // | HasReplacement e -> return e
        ...
        | FSharpExprPatterns.ThisValue typ ->
            ...
            return
                ...
                | RefType _, _ ->
                    tryGetIdentFromScopeIf ctx r None (fun fsRef ->
                        fsRef.IsConstructorThisValue)
                    |> Option.defaultWith (fun () ->
        ...
        | FSharpExprPatterns.Let((var, value, _), body) ->
            ...
            | _ when isInline var ->
                let ctx = {
                    ctx with
                        ScopeInlineValues = (var, value) :: ctx.ScopeInlineValues
                }
            ...
            | _ ->
                ...
                | Fable.Import(info, t, r) when not info.IsCompilerGenerated ->
                    return
                        Fable.Let(
                            ident,
                            Fable.Import(resolveImportMemberBinding ident info, t, r),
                            body
                        )
                ...
                | AST.NestedLambda(args, Fable.Import(info, _, r), _) when
                    ...
                    return
                        Fable.Let(
                            ident,
                            Fable.Extended(
                                Fable.Curry(Fable.Import(info, t, r), List.length args),
                                r
                            ),
                            body
                        )
        ...
        | FSharpExprPatterns.LetRec(recBindings, body) ->
            ...
            let ctx, idents =
                (recBindings, (ctx, []))
                ||> List.foldBack
                    (fun (PutIdentInScope com ctx (newContext, ident), _, _) (ctx, idents) ->
        ...
        | FSharpExprPatterns.CallWithWitnesses(callee,
            ...
            let callGenArgs =
                ownerGenArgs @ membGenArgs |> List.map (makeType ctx.GenericArgs)
                    ...
                    trampoline {
                        ...
                        | witnesses ->
                            ...
                            return!
                                ...
                                ||> trampolineListFold
                                    (fun ctx (traitName, isInstance, args, body) ->
                                        trampoline {
        ...
        | FSharpExprPatterns.Application(applied, genArgs, args) ->
            ...
            | FSharpExprPatterns.Let((_,
                                      FSharpExprPatterns.Call(None, m, _, _, [ e1; e2 ]),
                                      _),
                                     _),
              args when
        ...
        | FSharpExprPatterns.IfThenElse(guardExpr, thenExpr, elseExpr) ->
            ...
            return
                Fable.IfThenElse(guardExpr, thenExpr, altElseExpr, makeRangeFrom fsExpr)
        ...
        | FSharpExprPatterns.UnionCaseGet(IgnoreAddressOf unionExpr,
            ...
            | ErasedUnionCase ->
                let index =
                    unionCase.Fields |> Seq.findIndex (fun x -> x.Name = field.Name)
            ...
            | OptionUnion(t, _) ->
                return
                    Fable.Get(unionExpr, Fable.OptionValue, makeType ctx.GenericArgs t, r)
            | ListUnion t ->
            ...
            | DiscriminatedUnion(tdef, genArgs) ->
                ...
                let fieldIndex =
                    unionCase.Fields |> Seq.findIndex (fun fi -> fi.Name = field.Name)
        ...
        | FSharpExprPatterns.ValueSet(valToSet, valueExpr) ->
            ...
            | Some ent when
                ent.IsFSharpModule && isModuleValueCompiledAsFunction com valToSet
                ->
            ...
            | _ ->
                ...
                return
                    ...
                    | _ ->
                        Fable.Set(valToSet, Fable.ValueSet, valueExpr.Type, valueExpr, r)
        ...
        | FSharpExprPatterns.ObjectExpr(objType, baseCall, overrides, otherOverrides) ->
            ...
            | Some m when m.IsImplicitConstructor ->
                ...
                let ctx = {
                    ctx with
                        BoundConstructorThis = Some thisArg
                }
                ...
                | Some(baseEnt, captureBaseCall) ->
                    ...
                    | FSharpExprPatterns.Let(_, (ConstructorCall(call, genArgs, args))) ->
                        ...
                        | Some ent when ent = baseEnt ->
                            ...
                            transformBaseConsCall com ctx r baseEnt call genArgs args
                            |> captureBaseCall
        ...
        | FSharpExprPatterns.NewAnonRecord(fsType, argExprs) ->
            ...
            return
                Fable.NewAnonymousRecord(argExprs, fieldNames, genArgs, isStruct)
                |> makeValue r
            ...
            let compiledFableTargets =
                ...
                | RaisingMatchFailureExpr _infoWhereErrorOccurs ->
                    ...
                    | FSharpExprPatterns.IfThenElse(FSharpExprPatterns.UnionCaseTest(_unionValue,
                        ...
                        let errorMessage =
                            "Match failure: " + unionType.TypeDefinition.FullName
                        ...
                        let throwExpr =
                            makeThrow rangeOfLastDecisionTarget Fable.Any errorExpr
...
let private isIgnoredNonAttachedMember(memb: FSharpMemberOrFunctionOrValue) =
    ...
    || (match memb.DeclaringEntity with
        | Some ent ->
            isGlobalOrImportedFSharpEntity ent || isPojoDefinedByConsArgsFSharpEntity ent
        | None -> false)
...
let private applyJsPyDecorators
    ...
    let applyDecorator
        ...
        (attr:
            ...
            |})
        =
        let extraArgs = if attr.MethodInfo then [ methodInfo.Value ] else []
...
let private transformMemberDecl
    ...
    if isIgnoredNonAttachedMember memb then
        if
            memb.IsMutable && isNotPrivate memb && hasAttrib Atts.global_ memb.Attributes
        then
        ...
        if not memb.IsCompilerGenerated then
            ...
            | Some ent ->
                if isGlobalOrImportedFSharpEntity ent then
                    ...
                    |> Seq.tryHead
                    |> Option.iter (fun s ->
                        transformImplementedSignature com ctx ent s memb args body)
            | None -> ()
...
let resolveFieldType (ctx: Context) (entityType: FSharpType) (fieldType: FSharpType) =
    let entityGenArgs =
        ...
        | Some(tdef, _) when
            tdef.GenericParameters.Count = entityType.GenericArguments.Count
            ->
...
let resolveInlineExpr (com: IFableCompiler) ctx info expr =
    ...
    | Fable.Lambda(arg, b, n) ->
        Fable.Lambda(resolveInlineIdent ctx info arg, resolveInlineExpr com ctx info b, n)
    ...
    | Fable.TryCatch(b, c, d, r) ->
        Fable.TryCatch(
            ...
            (c
             |> Option.map (fun (i, e) ->
                 resolveInlineIdent ctx info i, resolveInlineExpr com ctx info e)),
    ...
    | Fable.Import(importInfo, t, r) as e ->
        ...
        if Path.isRelativePath importInfo.Path then
            ...
            if isImportToSameFile then
                Fable.IdentExpr {
                    makeTypedIdent t importInfo.Selector with
                        Range = r
                }
            else
    ...
    | Fable.Value(kind, r) as e ->
        ...
        | Fable.NewList(ht, t) ->
            let ht =
                ht
                |> Option.map (fun (h, t) ->
                    resolveInlineExpr com ctx info h, resolveInlineExpr com ctx info t)
        ...
        | Fable.BaseValue(i, t) ->
            ...
            |> makeValue r
        | Fable.TypeInfo(t, d) ->
            Fable.TypeInfo(resolveInlineType ctx.GenericArgs t, d) |> makeValue r
    ...
    | Fable.Unresolved(e, t, r) ->
        ...
        | Fable.UnresolvedTraitCall(sourceTypes, traitName, isInstance, argTypes, argExprs) ->
            ...
            | Some w ->
                ...
                let callee =
                    resolveInlineExpr com ctx { info with FileName = w.FileName } w.Expr
        ...
        | Fable.UnresolvedInlineCall(membUniqueName, witnesses, callee, callInfo) ->
            ...
            let ctx = {
                ctx with
                    Witnesses = witnesses @ ctx.Witnesses
            }
...
type FableCompiler(com: Compiler) =
    ...
    member this.AddConstructor
        (entityFullName, cons: Fable.MemberDecl, baseCall: Fable.Expr option)
        =
    ...
    member this.ResolveInlineExpr
        (ctx: Context, inExpr: InlineExpr, args: Fable.Expr list)
        =
        let rec foldArgs acc =
            ...
            | (argIdent: Fable.Ident) :: restArgIdents, [] ->
                foldArgs
                    ((argIdent,
                      Fable.Value(Fable.NewOption(None, argIdent.Type, false), None))
                     :: acc)
                    (restArgIdents, [])
            | [], _ -> List.rev acc
        ...
        let ctx, bindings =
            ...
            ||> List.fold (fun (ctx, bindings) (argId, arg) ->
                ...
                let ctx = {
                    ctx with
                        Scope = (None, argId, Some arg) :: ctx.Scope
                }
                ...
                isInline memb
                  ...
                  [
                      ...
                      InlineExprLazy(fun com ->
                          let com =
                              com.WillPrecompileInlineFunction(fileName) |> FableCompiler
                              :> IFableCompiler
                          ...
                          let genArgs =
                              memb.GenericParameters |> Seq.mapToList (genParamName)
