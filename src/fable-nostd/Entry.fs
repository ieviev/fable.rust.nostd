        ...
        result {
            ...
            let! projFile =
                if IO.Directory.Exists(fsprojPath) then
                    let files =
                        IO.Directory.EnumerateFileSystemEntries(fsprojPath) |> Seq.toList
            ...
            let startCompilation() =
                State.Create(
                    ...
                    useMSBuildForCracking = useMSBuildForCracking
                )
...
[<EntryPoint>]
let main argv =
    stdout.WriteLine "starting compiler:"
    ...
    result {
        ...
        | [ "watch" ] ->
            return! Runner.Run(args, language, rootDir, runProc, verbosity, watch = true)
        | [ "precompile"; path ] ->
            return!
                Runner.Run(
                    args,
        ...
        | [ "precompile" ] ->
            return!
                Runner.Run(args, language, rootDir, runProc, verbosity, precompile = true)
        | [ path ] ->
            return!
                Runner.Run(
                    args,
