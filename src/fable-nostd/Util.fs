...
module Log =
    ...
    let always (msg: string) = stdout.WriteLine msg

    let verbose (msg: Lazy<string>) =
        if isVerbose () then
            always msg.Value

    let warning (msg: string) = stdout.WriteLine msg

    let error (msg: string) = stderr.WriteLine msg

    let mutable private femtoMsgShown = false

    let info (msg: string) = stdout.WriteLine  msg
