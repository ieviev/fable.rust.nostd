...
type IRustCompiler =
    ...
    abstract GetImportName:
        Context * selector: string * path: string * SourceLocation option -> string
...
module Namespace =
    ...
    module Trie =
        ...
        let rec add path value trie =
            ...
            | [] -> {
                trie with
                    Values = Set.add value trie.Values
              }
            | x :: xs ->
                let child =
                    trie.Children
    ...
    let calcIdentUsages idents exprs =
        let usageCounts =
            ...
            |> List.map (fun (ident: Fable.Ident) ->
                let count =
                    exprs |> List.map (fun e -> countIdentUsage ident.Name e) |> List.sum
...
module TypeInfo =
    ...
    let makeBoxTy com ctx (ty: Rust.Ty) : Rust.Ty =
        [ ty ] |> mkGenericTy (splitNameParts "Box")
    ...
    let makeMutTy com ctx (ty: Rust.Ty) : Rust.Ty =
        ...
        {
            ty with
                kind = Fable.Transforms.Rust.AST.Types.TyKind.Rptr(None, muty)
        }
    ...
    let isPrintableEntity com entNames (ent: Fable.Entity) =
        (List.isEmpty ent.GenericParameters)
    ...
    let shouldBeRefCountWrapped (com: IRustCompiler) (ctx: Context) typ =
    ...
    let rec tryGetIdentName =
        ...
        | Fable.Operation(Fable.Unary(UnaryOperator.UnaryAddressOf, expr), _, _, _) ->
    ...
    let isDiscardedLambda(e: Fable.Expr) =
        match e with
        | Fable.Lambda(a, b, c) when a.IsCompilerGenerated -> Some(b)
        | _ -> None
    ...
    let transformOptionType com ctx genArg : Rust.Ty =
        // stdout.WriteLine $"opt: {genArg}"
        // transformAnyType com ctx
    ...
    let (|IsNonErasedInterface|_|)(com: Compiler) =
        ...
        | Fable.DeclaredType(entRef, genArgs) ->
            ...
            if
                ent.IsInterface && not (ent |> FSharp2Fable.Util.hasAttribute Atts.erase)
            then
                Some(entRef, genArgs)
            else
                None
    ...
    let transformAnyType com ctx : Rust.Ty =
        if ctx.InferAnyType then mkInferTy () else mkInferTy ()
    // failwith "any type"
    ...
    let isNullableReferenceType com ctx constraints =
        ...
        let isReferenceType =
            constraints |> List.contains Fable.Constraint.IsReferenceType
    ...
    let transformType (com: IRustCompiler) ctx (typ: Fable.Type) : Rust.Ty =
        let ty =
            ...
            | Fable.AnonymousRecordType(fieldNames, genArgs, isStruct) ->
                transformTupleType com ctx isStruct genArgs
            | Replacements.Util.IsEntity (Types.nullable) (entRef, [ genArg ]) ->
                transformNullableType com ctx genArg
            ...
            | Replacements.Util.IsEntity (Types.valueCollection) (entRef, [ k; v ]) ->
                transformArrayType com ctx v
            | Replacements.Util.IsEntity (Types.icollectionGeneric) (entRef, [ t ]) ->
        ...
        else if (isByRefType com typ || ctx.IsParamByRefPreferred) then
            ty |> mkRefTy None
        else
            match shouldBeRefCountWrapped com ctx typ with
            // | Some Lrc -> ty |> makeRcTy com ctx
            | Some Rc -> ty |> makeRcTy com ctx
            // | Some Lrc -> ty |> mkRefTy None
            // | Some Rc -> ty |> mkRefTy None
...
module Util =
    ...
    let (|IComparable|_|) =
        function
        | Replacements.Util.IsEntity (Types.icomparableGeneric) (_, [ genArg ]) ->
            Some(genArg)
        | _ -> None
    ...
    let getExpr r (expr: Rust.Expr) (index: Rust.Expr) =
        mkIndexExpr expr index |> mkAddrOfExpr // ?loc=r)
        ...
        | _ ->
            ...
            let ctx =
                if ctx.IsLambda then
                    ...
                    {
                        ctx with
                            ScopedMemberGenArgs = genArgNames
                    }
    ...
    let transformCast
        ...
        | t, Fable.Any when isValueType com t || isWrappedType com t ->
            expr |> boxValue com ctx

        // unboxing value types or wrapped types
        | Fable.Any, t when isValueType com t || isWrappedType com t ->
            expr |> unboxValue com ctx t
        ...
        | t1, t2 when not (isInterface com t1) && (isInterface com t2) ->
            makeInterfaceCast com ctx t2 expr
    ...
    let makeLrcPtrValue com ctx (value: Rust.Expr) =
        [ value ] |> makeNew com ctx "std::rc" "Rc"
    ...
    let parameterIsByRefPreferred idx (parameters: Fable.Parameter list) =
        ...
        |> List.tryItem idx
        |> Option.map (fun p ->
            p.Attributes |> Seq.exists (fun a -> a.Entity.FullName = Atts.rustByRef))
        |> Option.defaultValue false
    ...
    let negateWhen isNegative expr =
        if isNegative then expr |> mkNegExpr else expr

    let makeNumber com ctx r t (v: Fable.NumberValue) =
        match v with
        | Fable.NumberValue.Int8 x when x = System.SByte.MinValue ->
            mkGenericPathExpr ("i8" :: "MIN" :: []) None
        | Fable.NumberValue.Int8 x when x = System.SByte.MaxValue ->
            mkGenericPathExpr ("i8" :: "MAX" :: []) None
        | Fable.NumberValue.Int16 x when x = System.Int16.MinValue ->
            mkGenericPathExpr ("i16" :: "MIN" :: []) None
        | Fable.NumberValue.Int16 x when x = System.Int16.MaxValue ->
            mkGenericPathExpr ("i16" :: "MAX" :: []) None
        | Fable.NumberValue.Int32 x when x = System.Int32.MinValue ->
            mkGenericPathExpr ("i32" :: "MIN" :: []) None
        | Fable.NumberValue.Int32 x when x = System.Int32.MaxValue ->
            mkGenericPathExpr ("i32" :: "MAX" :: []) None
        | Fable.NumberValue.Int64 x when x = System.Int64.MinValue ->
            mkGenericPathExpr ("i64" :: "MIN" :: []) None
        | Fable.NumberValue.Int64 x when x = System.Int64.MaxValue ->
            mkGenericPathExpr ("i64" :: "MAX" :: []) None
        | Fable.NumberValue.UInt8 x when x = System.Byte.MaxValue ->
            mkGenericPathExpr ("u8" :: "MAX" :: []) None
        | Fable.NumberValue.UInt16 x when x = System.UInt16.MaxValue ->
            mkGenericPathExpr ("u16" :: "MAX" :: []) None
        | Fable.NumberValue.UInt32 x when x = System.UInt32.MaxValue ->
            mkGenericPathExpr ("u32" :: "MAX" :: []) None
        | Fable.NumberValue.UInt64 x when x = System.UInt64.MaxValue ->
            mkGenericPathExpr ("u64" :: "MAX" :: []) None
        | Fable.NumberValue.Float32 x when System.Single.IsNaN(x) ->
            mkGenericPathExpr ("f32" :: "NAN" :: []) None
        | Fable.NumberValue.Float64 x when System.Double.IsNaN(x) ->
        ...
        | Fable.NumberValue.Int128(upper, lower) ->
            let bytes =
                Array.concat [
                    BitConverter.GetBytes(lower)
                    BitConverter.GetBytes(upper)
                ] // little endian
    ...
    let makeArrayFrom (com: IRustCompiler) ctx r typ fableExpr =
        ...
        | Fable.Value(Fable.NewTuple([ valueExpr; countExpr ], isStruct), _) ->
            ...
            makeLibCall com ctx None "NativeArray" "new_init" [
                value
                count
            ]
        | expr ->
    ...
    let makeTuple (com: IRustCompiler) ctx r isStruct (exprs: (Fable.Expr) list) =
        ...
        if isStruct then expr else expr
    ...
    let tryUseKnownUnionCaseNames fullName =
        ...
        | _ ->
            if
                fullName.StartsWith("FSharp.Core.FSharpChoice`", StringComparison.Ordinal)
            then
    ...
    let transformValue (com: IRustCompiler) (ctx: Context) r value : Rust.Expr =
        ...
        | Fable.StringConstant s ->
            let strlit = mkStrLitExpr s
            mkMethodCallExpr "to_string" None strlit []
        ...
        | Fable.NewArray((Fable.ArrayFrom expr | Fable.ArrayAlloc expr), typ, _kind) ->
            makeArrayFrom com ctx r typ expr
        | Fable.NewTuple(values, isStruct) -> makeTuple com ctx r isStruct values
        | Fable.NewList(headAndTail, typ) -> failwith "list unsupported"
    ...
    let makeObjectExprMemberDecl (com: IRustCompiler) ctx (memb: Fable.ObjectExprMember) =
        ...
        let replacements =
            ...
            |> Map.map (fun _name (ident: Fable.Ident) ->
                let info =
                    Fable.FieldInfo.Create(ident.Name, ident.Type, ident.IsMutable)
    ...
    let transformObjectExpr
        ...
        if members |> List.isEmpty then
            ...
            let members: Fable.MemberDecl list =
                members
                |> List.map (fun memb ->
                    let memberDecl, capturedIdents =
                        makeObjectExprMemberDecl com ctx memb
        ...
        | Fable.Binary(op, leftExpr, rightExpr) ->
            let kind =
                ...
                | BinaryOperator.BinaryExponent ->
                    failwithf "BinaryExponent not supported. TODO: implement with pow."
            ...
            | _ ->
                let left =
                    transformLeaveContext com ctx None leftExpr |> maybeAddParens leftExpr
    ...
    let transformEmit (com: IRustCompiler) ctx range (emitInfo: Fable.EmitInfo) =
        ...
        if macro.EndsWith("!", StringComparison.Ordinal) then
            ...
            let thisArgOpt =
                info.ThisArg |> Option.map (fun e -> com.TransformExpr(ctx, e))
    ...
    let replaceCall
        ...
        : AST.Types.Expr voption =
        // stdout.WriteLine "transform call"
    ...
    let transformCall
        ...
        (callInfo: Fable.CallInfo)
        : AST.Types.Expr =
        match replaceCall com ctx range typ calleeExpr callInfo with
        | ValueSome e -> e
        | _ ->
        ...
        // stdout.WriteLine $"cal;: {calleeExpr} : {membOpt}"
    ...
    let mutableSet expr value = mkAssignExpr expr value
    ...
    let transformGet (com: IRustCompiler) ctx range typ (fableExpr: Fable.Expr) kind =
        ...
        | Fable.ExprGet idx ->
            ...
            | _ -> getExpr range expr prop // |> makeClone
        ...
        | Fable.FieldGet info ->
            ...
            | _ ->
                ...
                if info.IsMutable then field |> mutableGet else field

        | Fable.ListHead -> failwith $"unsupported"
        | Fable.ListTail -> failwith $"unsupported"
    ...
    let transformSet
        ...
        | Fable.ValueSet ->
            ...
            | Fable.Call(Fable.IdentExpr ident, info, _, _) when
                ident.IsMutable && isModuleMemberCall com info
                ->
                let expr = transformIdent com ctx range ident
                mutableSet (mkCallExpr expr []) value
            // mutable idents captured in object expression methods
            | Fable.Get(Fable.Value(Fable.ThisValue _, _) as thisArg,
                        Fable.GetKind.FieldGet info,
                        _t,
                        _r) when info.IsMutable ->
    ...
    let makeLetStmt com ctx (ident: Fable.Ident) value isCaptured usages =
        ...
        let tyOpt: AST.Types.Ty option =
            // stdout.WriteLine $"tyo: {value}"
            match value with
            | Fable.Emit(s, t, r) -> Some(transformType com ctx t)
    ...
    let makeLetStmts (com: IRustCompiler) ctx bindings letBody usages =
        ...
        let ctx, letStmtsRev =
            ...
            ||> List.fold (fun (ctx, lst) (ident: Fable.Ident, value) ->
                ...
                // stdout.WriteLine $"let: {isCaptured}"
                let stmt, ctxNext =
                    ...
                    | Function(args, body, _name) when not (ident.IsMutable) ->
                        if hasCapturedIdents com ctx ident.Name args body then
                            ...
                            makeLetStmt
                                com
                                ctx
                                { ident with IsMutable = true }
                                value
                                isCaptured
                                usages
                        else
                            // stdout.WriteLine $"fadf: {isCaptured}"
    ...
    let transformIfThenElse (com: IRustCompiler) ctx range guard thenBody elseBody =
        let guardExpr =
            ...
            | Fable.Test(expr, Fable.OptionTest isSome, r) ->
                makeOptionTest com ctx r isSome expr
            | Fable.Test(expr, Fable.UnionCaseTest tag, r) ->
                makeUnionCaseTest com ctx r tag expr
    ...
    let transformForLoop
        ...
        let rangeExpr =
            if isUp then
                ...
                // downward loop
                let rangeExpr =
                    mkRangeExpr (Some limitExpr) (Some startExpr) true |> mkParenExpr
    ...
    let transformTryCatch
        ...
        | Some(catchVar, catchBody) ->
            ...
            makeLibCall com ctx None "Exception" "try_catch" [
                try_f
                catch_f
            ]
    ...
    let transformCurriedApply
        ...
        (args: Fable.Expr list)
        =
        let disclambda = isDiscardedLambda calleeExpr

        if disclambda.IsSome then
            transformExpr com ctx disclambda.Value
        else

        match ctx.TailCallOpportunity with
        // | Some tc when
        //     tc.IsRecursiveRef(calleeExpr) && List.length tc.Args = List.length args
        //     ->
    ...
    let transformTypeTest
        ...
        | Fable.IdentExpr ident when isDowncast ->
            let downcastExpr =
                makeLibCall com ctx genArgsOpt "Native" "try_downcast" [ expr ]

            let pat =
                makeUnionCasePat (rawIdent "Some") [ makeFullNameIdentPat ident.Name ]
    ...
    let makeTest isSome thenValue elseValue = if isSome then thenValue else elseValue
    ...
    let transformSwitch
        ...
        let makeArm
            ...
            let idents, (bodyExpr: Fable.Expr) =
                ctx.DecisionTargets |> List.item targetIndex // TODO:
            ...
            let body =
                ...
                let extraIdents =
                    extraVals
                    |> List.map (fun (_name, friendlyName, typ) ->
                        makeTypedIdent typ friendlyName)
        ...
        let arms =
            ...
            |> List.map (fun (caseExpr, targetIndex, boundValues) ->
                let patOpt =
                    match caseExpr with
                    | Fable.Value(Fable.NumberConstant(Fable.NumberValue.Int32 tag,
                                                       Fable.NumberInfo.Empty),
                                  r) -> makeUnionCasePatOpt com ctx evalType evalName tag
                    | _ -> None
    ...
    let canTransformDecisionTreeAsSwitch guardExpr thenExpr elseExpr =
        ...
        let rec sameEvalExprs evalExpr1 evalExpr2 =
            ...
            | Fable.Get(e1, Fable.TupleIndex i1, _, _),
              Fable.Get(e2, Fable.TupleIndex i2, _, _) -> i1 = i2 && sameEvalExprs e1 e2
        ...
        |> visitFromInsideOut (function
            | Fable.IfThenElse(guardExpr1,
                               Fable.IfThenElse(guardExpr2,
                                                thenExpr,
                                                Fable.DecisionTreeSuccess(index2, [], _),
                                                _),
    ...
    let getTargetsWithMultipleReferences expr =
        ...
        |> Seq.choose (fun kv -> if kv.Value > 1 then Some kv.Key else None)
        |> Seq.toList
    ...
    let isBin(com: IRustCompiler) =
        isLastFileInProject com
        && Path.GetFileNameWithoutExtension com.CurrentFile = "main"

    let getModuleItems (com: IRustCompiler) ctx =
        let outDir =
            com.OutputDir
            |> Option.defaultWith (fun () ->
                System.IO.Path.GetDirectoryName(com.CurrentFile))
        ...
        if isLastFileInProject com then
            ...
            let rawModItems =
                rawmods |> Seq.map (fun v -> mkUnloadedModItem [] v) |> Seq.toList
    ...
    let tryFindClosedOverIdent com ctx (ignoredNames: HashSet<string>) expr =
        ...
        | Fable.LetRec(bindings, _) ->
            bindings
            |> List.iter (fun (ident, _) -> ignoredNames.Add(ident.Name) |> ignore)

            None
        | Fable.DecisionTree(_, targets) ->
            targets
            |> List.iter (fun (idents, _) ->
                idents |> List.iter (fun ident -> ignoredNames.Add(ident.Name) |> ignore))
    ...
    //         args
    //         |> List.filter (fun arg ->
    //             not (arg.IsMutable || arg.IsThisArgument && ctx.IsAssocMember)
    //         )
    ...
    let transformLambda
        ...
        (body: Fable.Expr)
        =
        // todo: this should be inlined in the outer fn as well then
        // stdout.WriteLine $"lambda(emit): {body.IsEmit}"
        // stdout.WriteLine $"argv: %A{args}"
        // if args |> Seq.forall (fun v -> v.IsCompilerGenerated) then
        //     // stdout.WriteLine $"lambda(emit): {body.IsEmit}"
        //     // match body with
        //     // | Fable.CurriedApply(applied, expr, typ, range) ->
        //     //     let e1 = transformExpr com ctx expr[1]
        //     //     e1
        //     // | e ->
        //     transformExpr com ctx body
        // else
        // stdout.WriteLine $"lambdaname: {name}, idents: {args}"
    ...
    let makeTypeBounds
        ...
        let makeConstraint c =
            ...
            | Fable.Constraint.IsNullable -> [
                makeImportBound com ctx "Native" "NullableRef"
              ]
    ...
    let makeFnHeader com ctx (attributes: Fable.Attribute seq) : Rust.FnHeader =
        let isAsync =
            attributes |> Seq.exists (fun a -> a.Entity.FullName = Atts.rustAsync)

        let isConst =
            attributes |> Seq.exists (fun a -> a.Entity.FullName = Atts.rustConst)
    ...
    let transformAttributes
        ...
        |> Seq.collect (fun a ->
            ...
            // TODO: support more test frameworks
                ...
                a.Entity.FullName.EndsWith(".FactAttribute", StringComparison.Ordinal)
            then
                [ mkAttr "test" [] ]
            else
                [])
    ...
    let transformModuleFunction
        ...
        let fnBodyBlock =
            if isUnsafe then fnBody |> mkUnsafeBlockExpr |> mkBodyBlock
            else if decl.Body.Type = Fable.Unit then mkSemiBlock fnBody
    ...
    let maybeConstructorBaseCall (com: IRustCompiler) (body: Fable.Expr) =
        ...
        | Fable.Sequential([ first; second ]) ->
            ...
            | Fable.Call(_, info, typ, _),
              Fable.Value(Fable.NewRecord(values, entRef, genArgs), r) ->
                let membOpt = info.MemberRef |> Option.bind com.TryGetMember

                match membOpt, typ with
                | Some memb, Fable.DeclaredType(baseEntRef, _) when
                    memb.IsConstructor && isValidBaseType com baseEntRef
                    ->
    ...
    let transformPrimaryConstructor
        ...
        let body =
            ...
            | Fable.Sequential exprs ->
                ...
                let fieldIdents =
                    idents |> List.map (fun ident -> Map.find ident.Name identMap)
                ...
                let retVal =
                    Fable.Value(Fable.NewRecord(fieldValues, ent.Ref, genArgs), None)
                ...
                let body =
                    ...
                    |> visitFromInsideOut (function
                        | Fable.Set(Fable.Value(Fable.ThisValue _, _),
                                    ...
                                    r) ->
                            let identExpr =
                                identMap |> Map.find fieldName |> Fable.IdentExpr
                        ...
                        | Fable.Get(Fable.Value(Fable.ThisValue _, _),
                                    ...
                                    r) ->
                            let identExpr =
                                identMap |> Map.find info.Name |> Fable.IdentExpr
    ...
    let transformInterface
        ...
        let implItem =
            ...
            let ty = mkGenericTy [ typeName ] [] |> makeRcTy com ctx
    ...
    let makeDerefTraitImpls
        com
        ctx
        entName
        genArgs
        (baseType: Fable.Type option)
        (bodyExpr: string)
        =
        // expected output:
        ...
        // }
        match baseType with
        | Some(Fable.DeclaredType(baseEntRef, baseGenArgs)) when
            isValidBaseType com baseEntRef
            ->
            let baseTy = transformEntityType com ctx baseEntRef baseGenArgs
            let targetTy = baseTy |> makeRcTy com ctx
    ...
    let makeDisplayTraitImpls com ctx entName genArgs hasToString hasDebug =
        ...
        let fnDecl =
            ...
            let output =
                let ty =
                    mkGenericPathTy ("core" :: "fmt" :: (rawIdent "Result") :: []) None
        ...
        then
            ...
            let ctorItem =
                ...
                | Some ctor ->
                    ...
                    <| fun ctx ->
                        transformPrimaryConstructor com ctx ent ctor decl.BaseCall
    ...
    let findInterfaceGenArgs
        (com: IRustCompiler)
        (ent: Fable.Entity)
        (ifcEntRef: Fable.EntityRef)
        =
        let ifcOpt =
            ent.AllInterfaces
    ...
    let transformClassMembers
        ...
        let ctx = {
            ctx with
                ScopedEntityGenArgs = getEntityGenParamNames ent
        }

        let isIgnoredMember(memb: Fable.MemberFunctionOrValue) =
            ent.IsFSharpExceptionDeclaration // to filter out compiler-generated exception equality
        ...
        for entry in sorted do
            ...
            |> Seq.iter (fun import ->
                ...
                | _ ->
                    ...
                    let alias =
                        if Array.last parts <> import.LocalIdent then
                            Some(import.LocalIdent)
                        else
                            None

                    let useItem = mkSimpleUseItem [] (Array.append modPath parts) alias
                    items.Add(useItem)
                    ())
    ...
    let getIdentForImport (ctx: Context) (path: string) (selector: string) =
        ...
        | _ -> splitNameParts selector |> Array.last
...
module Compiler =
    ...
    type RustCompiler(com: Fable.Compiler) =
        ...
        interface IRustCompiler with
            ...
            member self.GetImportName(ctx, selector, path, r) =
                ...
                let cacheKey =
                    if path.Length = 0 then selector else path + "::" + selector
                ...
                let import =
                    ...
                    | false, _ ->
                        ...
                        if
                            path.Length > 0
                            && not (modulePath.Contains("RUST_LIBRARY_PATH"))
                        then
    ...
    let transformFile (com: Fable.Compiler) (file: Fable.File) =
        ...
        if is_main then
            ...
            let items = [
                for m in firstmod |> Seq.skip 1 do // skip use super *
                    mkItem m.attrs m.ident m.kind
            ]

            let crateItems = importItems @ items //@ modItems //@ nsItems
            ...
            let crateItems = importItems @ declItems @ modItems @ nsItems
