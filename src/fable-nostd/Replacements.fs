...
module Helpers =
    ...
    let moduleCallIndex
        ...
        let info =
            CallInfo.Create(args = args, ?sigArgTypes = argTypes, ?genArgs = genArgs)
        // Call(callee, info, returnType, loc)
        Call(callee, info, returnType, loc)

    let indexExpr = 1

let inline mk_module_call r com thisArg t args moduleName memberName =
...
let toLowerFirstWithArgsCountSuffix (args: Expr list) meth =
    ...
    if argCount > 1 then meth + (string argCount) else meth
...
let convertTo com (ctx: Context) r t (args: Expr list) =
    ...
    | Char ->
        ...
        | Number(Decimal, _) ->
            Helper.LibCall(com, "Decimal", "toChar", t, args, ?loc = r)
...
let emitFormat (com: ICompiler) r t (args: Expr list) macro =
    let args =
        match args with
        | [] -> [ emitRawString "" ]
        | [ StringConst fmt; Value(NewArray(ArrayValues restArgs, _, _), _) ] ->
            (emitRawString fmt) :: restArgs
...
let referenceEquals (com: ICompiler) ctx r (left: Expr) (right: Expr) =
    ...
    | Value(Null _, _), o
    | o, Value(Null _, _) ->
        Helper.LibCall(com, "Native", "is_null", Boolean, [ o ], ?loc = r)
    | _ ->
...
let compare (com: ICompiler) ctx r (left: Expr) (right: Expr) =
    ...
    | String
    | Number _
    | Builtin(FSharpChoice _ | FSharpResult _) ->
        Helper.LibCall(
            com,
            "Native",
            "compare",
...
let makeEqualityComparer (com: ICompiler) ctx typArg =
    ...
    objExpr [
        ...
        "GetHashCode",
        Delegate([ x ], getHashCode com ctx None (IdentExpr x), None, Tags.empty)
    ]
...
let rec getZero (com: ICompiler) (ctx: Context) (t: Type) =
    ...
    | IsEntity (Types.nullable) (_entRef, [ genArg ]) ->
        NewOption(None, genArg, false) |> makeValue None
...
let makeAddFunction (com: ICompiler) ctx t =
    ...
    let body =
        applyOp com ctx None t Operators.addition [
            IdentExpr x
            IdentExpr y
        ]
...
let fableCoreLib
    ...
    | "Fable.Core.Rust", _ ->
        match i.CompiledName, args with
        | "import",
          [ RequireStringConst com ctx r selector; RequireStringConst com ctx r path ] ->
    ...
    let rustFmt =
        Regex.Replace(
            ...
            fun m ->
                ...
                let g4 = if g4 = "" && (g5 = "f" || g5 = "F") then ".6" else g4
...
let makeRustFormatExpr com r t (fmt: string) args macro =
    // let macroExpr = Helper.LibValue(com, "", macro, Any)
    // let expr = Helper.ImportedValue(com, "", macro, Any)
    // emitExpr None String []
    let rustFmt, argCount = makeRustFormatString false fmt
    // let argCount = argCount + 1 + (List.length args) // +1 is for fmt
    // let applied = Extended(Curry(exp, argCount), r)
    // curriedApply r t applied (args @ [ emitRawString rustFmt ])

    // let exp = emitExpr None Unit [] macro
    
    emitExpr None t (args @ [ emitRawString rustFmt ]) macro

    // emitExpr None Unit [] macro
    // emitRawString rustFmt
...
let fsFormat
    ...
    | "PrintFormatLineToTextWriter", None, _ ->
        "printfn!" |> emitFormat com r t args |> Some
    | "PrintFormatToStringThenFail", None, [ StringConst fmt ] ->
        "failwithf!" |> makeRustFormatExpr com r t fmt [] |> Some
    | "PrintFormatToStringThenFail", None, _ ->
        "failwithf!" |> emitFormat com r t args |> Some
...
let operators
    ...
    | ("Failure" | "FailurePattern" | "LazyPattern" | "NullArg" | "Using"), _ ->
    ...
    | ("Min" | "Max" as meth), _ ->
        ...
        | _ ->
            Helper.LibCall(com, "Native", Naming.lowerFirst meth, t, args, ?loc = r)
            |> Some
...
let chars
    ...
    | ("ConvertFromUtf32" | "ToLower" | "ToLowerInvariant" | "ToUpper" | "ToUpperInvariant" as meth),
      None,
      [ c ] ->
        Helper.LibCall(com, "Char", meth, t, args, i.SignatureArgTypes, ?loc = r)
        |> Some
    | ("TryParse" | "Parse" as meth), None, _ ->
        Helper.LibCall(com, "Char", meth, t, args, i.SignatureArgTypes, ?loc = r)
        |> Some
    | ("IsSurrogate" | "IsHighSurrogate" | "IsLowSurrogate" | "IsSurrogatePair" as meth),
      None,
      _ ->
        ...
        Helper.LibCall(com, "Char", meth, t, args, i.SignatureArgTypes, ?loc = r)
        |> Some
    | ("Compare" | "CompareTo" | "Equals" | "GetHashCode"), _, _ ->
...
let strings
    ...
    | "Contains", Some c, _ ->
        ...
        | [ ExprType String; _comparison ] ->
            Helper.LibCall(com, "String", "contains2", t, c :: args, ?loc = r) |> Some
        | _ -> None
    | "EndsWith", Some c, _ ->
        match args with
        | [ ExprType Char ] ->
        ...
        | [ ExprType String; _comparison ] ->
            Helper.LibCall(com, "String", "endsWith2", t, c :: args, ?loc = r) |> Some
    ...
    | ("IndexOf" | "LastIndexOf" | "IndexOfAny" | "LastIndexOfAny"), Some c, _ ->
        let suffixOpt =
            ...
            | [ ExprType String; ExprType(Number(Int32, _)); ExprType(Number(Int32, _)) ] ->
                Some "3"
            | [ ExprType Char ] -> Some "Char"
            | [ ExprType Char; ExprType(Number(Int32, _)) ] -> Some "Char2"
            | [ ExprType Char; ExprType(Number(Int32, _)); ExprType(Number(Int32, _)) ] ->
                Some "Char3"
    ...
    | "IsNullOrEmpty", None, _ ->
        Helper.LibCall(com, "String", "isNullOrEmpty", t, args, ?loc = r) |> Some
    | "IsNullOrWhiteSpace", None, _ ->
        Helper.LibCall(com, "String", "isNullOrWhitespace", t, args, ?loc = r) |> Some
    | "Join", None, _ ->
        let args =
            match args with
    ...
    | "Split", Some c, _ ->
        ...
        | [ Value(NewArray(ArrayValues [ arg1 ], String, _), _)
            ...
            |> Some
        | [ ExprTypeAs(Array(String, _), arg1)
            ExprTypeAs(Number(_, NumberInfo.IsEnum _), arg2) ] ->
            Helper.LibCall(
                com,
                "String",
    ...
    | "StartsWith", Some c, _ ->
        match args with
        | [ ExprType Char ] ->
            Helper.LibCall(com, "String", "startsWithChar", t, c :: args, ?loc = r)
            |> Some
        | [ ExprType String ] ->
    ...
    | ("Trim" | "TrimStart" | "TrimEnd"), Some c, _ ->
        ...
        | [] -> Helper.LibCall(com, "String", methName, t, c :: args, ?loc = r) |> Some
        | [ ExprType Char ] ->
            Helper.LibCall(com, "String", methName + "Char", t, c :: args, ?loc = r)
            |> Some
        | [ ExprType(Array(Char, _)) ] ->
...
let formattableString
    ...
    | "Create", None, [ StringConst str; Value(NewArray(ArrayValues args, _, _), _) ] ->
        let matches =
            Regex.Matches(str, @"\{\d+(.*?)\}") |> Seq.cast<Match> |> Seq.toArray
        ...
        let holes =
            matches |> Array.map (fun m -> {| Index = m.Index; Length = m.Length |})
...
let resizeArrays
    ...
    match i.CompiledName, thisArg, args with
    | ".ctor", _, [] ->
        Some(mk_module_call r com thisArg t args "std::vec" "Vec::new")
    | ".ctor", _, [ ExprType(Number(Int32, _)) ] ->
        Some(mk_module_call r com thisArg t args "std::vec" "Vec::new")
    | ".ctor", _, [ ExprType(IEnumerable) ] ->
        Some(mk_module_call r com thisArg t args "std::vec" "Vec::new")
    | ".ctor", _, [ arg ] -> toArray com t arg |> Some
    | "get_Capacity", Some ar, _ ->
        Helper.LibCall(com, "NativeArray", "get_Capacity", t, [ ar ], ?loc = r) |> Some
    | "get_Count", Some this, _ ->
        let rettype = Fable.AST.Fable.Type.Number(NumberKind.UNativeInt,NumberInfo.Empty)
        Some(Helper.InstanceCall(this, "len", rettype, [Expr.Value(ValueKind.UnitConstant, None)]))
    ...
    | "Add", Some this, _ ->
        ...
        Some(Helper.InstanceCall(this, "push", Unit, args))
    | ("AddRange" | "Clear" | "Contains" | "ConvertAll" | "Exists" | "GetRange" | "Slice" | "ForEach" | "FindAll" | "Find" | "FindLast" | "FindIndex" | "FindLastIndex" | "Insert" | "InsertRange" | "Remove" | "RemoveAt" | "RemoveAll" | "RemoveRange" | "ToArray" | "TrimExcess" | "TrueForAll") as meth,
      Some ar,
      args ->
        // methods without overrides
        let meth = Naming.lowerFirst meth
        Helper.LibCall(com, "NativeArray", meth, t, (ar :: args), ?loc = r) |> Some
    | ("BinarySearch" | "CopyTo" | "IndexOf" | "LastIndexOf" | "Reverse") as meth,
      Some ar,
      args ->
        ...
        Helper.LibCall(com, "NativeArray", meth, t, (ar :: args), ?loc = r) |> Some
    | "Sort", Some ar, [] ->
        Helper.LibCall(com, "NativeArray", "sort", t, (ar :: args), ?loc = r) |> Some
...
let arrays
    ...
    | "get_Length", Some ar, _ ->
        Helper.LibCall(com, "NativeArray", "get_Count", t, [ ar ], ?loc = r) |> Some
    | "get_Item", Some ar, [ idx ] -> getExpr r t ar idx |> Some
    | "set_Item", Some ar, [ idx; value ] -> setExpr r ar idx value |> Some
    | "Clone", Some ar, _ ->
        Helper.LibCall(com, "NativeArray", "toArray", t, [ ar ], ?loc = r) |> Some
    | "Copy", None, [ _source; _sourceIndex; _target; _targetIndex; _count ] ->
        copyToArray com r t i args
    | "Copy", None, [ source; target; count ] ->
        copyToArray com r t i [
            source
            makeIntConst 0
            target
            makeIntConst 0
    ...
    | ("BinarySearch" | "CopyTo" | "IndexOf" | "LastIndexOf" | "Reverse") as meth,
      ...
      args ->
        ...
        Helper.LibCall(com, "NativeArray", meth, t, args, ?loc = r) |> Some
    | "Sort", None, [ ar ] ->
        Helper.LibCall(com, "NativeArray", "sort", t, args, ?loc = r) |> Some
    | "Sort", None, [ ar; ExprType(DelegateType _) as comparer ] ->
        Helper.LibCall(com, "NativeArray", "sortBy", t, args, ?loc = r) |> Some
    | "Sort", None, [ ar; comparer ] ->
        Helper.LibCall(com, "NativeArray", "sortWith", t, args, ?loc = r) |> Some
    | _ -> None
...
let arrayModule
    ...
    | "Copy", [ ar ] ->
        Helper.LibCall(com, "NativeArray", "toArray", t, args, ?loc = r) |> Some
...
let lists
    ...
    | "Cons", None, [ h; t ] ->
        NewList(Some(h, t), (genArg com ctx r 0 i.GenericArgs)) |> makeValue r |> Some
...
let nullables
    ...
    match i.CompiledName, thisArg with
    | ".ctor", None ->
        NewOption(List.tryHead args, t.Generics.Head, false) |> makeValue r |> Some
    // | "get_Value", Some c -> Get(c, OptionValue, t, r) |> Some // Get(OptionValue) doesn't do a null check
    | "get_Value", Some c ->
        Helper.LibCall(com, "Option", "getValue", t, [ c ], ?loc = r) |> Some
...
let options
    ...
    match i.CompiledName, thisArg with
    | "Some", _ ->
        NewOption(List.tryHead args, t.Generics.Head, isStruct) |> makeValue r |> Some
...
let optionModule
    ...
    | "OfObj", [ arg ] ->
        Helper.LibCall(com, "Native", "ofObj", t, args, ?loc = r) |> Some
    | "ToObj", [ arg ] ->
        Helper.LibCall(com, "Native", "toObj", t, args, ?loc = r) |> Some
...
let parseBool
    ...
    | ("Parse" | "TryParse" as method), args ->
        ...
        |> Some
    | ("Compare" | "CompareTo" | "Equals" | "GetHashCode"), _ ->
        valueTypes com ctx r t i thisArg args
    | _ -> None
...
let parseNum
    ...
    let parseCall meth str args style =
        ...
        let outValue = if meth = "TryParse" then [ List.last args ] else []
...
let decimals
    ...
    | (".ctor" | "MakeDecimal"), ([ low; mid; high; isNegative; scale ] as args) ->
        ...
        |> Some
    | ".ctor",
      [ Value(NewArray(ArrayValues([ low; mid; high; signExp ] as args), _, _), _) ] ->
...
let languagePrimitives
    ...
    | ("FastCompareTuple2" | "FastCompareTuple3" | "FastCompareTuple4" | "FastCompareTuple5" | "GenericComparisonWithComparer" | "GenericComparisonWithComparerIntrinsic"),
      [ comp; left; right ] ->
        makeInstanceCall r t i comp "Compare" [
            left
            right
        ]
        |> Some
    ...
    | ("FastEqualsTuple2" | "FastEqualsTuple3" | "FastEqualsTuple4" | "FastEqualsTuple5" | "GenericEqualityWithComparer" | "GenericEqualityWithComparerIntrinsic"),
      [ comp; left; right ] ->
        makeInstanceCall r t i comp "Equals" [
            left
            right
        ]
        |> Some
...
let dictionaries
    ...
    | ".ctor", None ->
        match i.SignatureArgTypes with
        | [] ->
            Some(mk_module_call r com thisArg t args "std::collections" "HashMap::new")
        | [ Number _ ] ->
            Some(mk_module_call r com thisArg t args "std::collections" "HashMap::new")
        | [ IEnumerable ] ->
            Some(mk_module_call r com thisArg t args "std::collections" "HashMap::new")
...
let hashSets
    ...
    | ".ctor", None ->
        ...
        | [ IEqualityComparer ] ->
            Helper.LibCall(com, "HashSet", "new_with_comparer", t, args) |> Some
        | [ Number _; IEqualityComparer ] ->
            Helper.LibCall(com, "HashSet", "new_with_capacity_comparer", t, args) |> Some
        | [ IEnumerable ] ->
            Helper.LibCall(com, "HashSet", "new_from_enumerable", t, args) |> Some
        | [ IEnumerable; IEqualityComparer ] ->
            Helper.LibCall(com, "HashSet", "new_from_enumerable_comparer", t, args)
            |> Some
        | _ -> None
    | "GetEnumerator", Some c ->
...
let bitConvert
    ...
    | "GetBytes" ->
        let memberName =
            ...
            | x ->
                FableError $"Unsupported type in BitConverter.GetBytes(): %A{x}" |> raise
...
let convert
    ...
    | ("ToHexString" | "ToHexStringLower" | "FromHexString" | "ToBase64String" | "FromBase64String"),
      [ arg ] ->
        Helper.LibCall(
            com,
            "Convert",
...
let asyncBuilder
    ...
    | "Using", Some callee, [ arg; f ] ->
        makeInstanceCall r t i callee "Using" [
            arg
            f
        ]
        |> Some
...
let asyncs com (ctx: Context) r t (i: CallInfo) (_: Expr option) (args: Expr list) =
    ...
    | "get_CancellationToken" ->
        Helper.LibCall(com, "Async", "cancellationToken", t, [], ?loc = r) |> Some
    ...
    let resolved =
        ...
        | Some(Value(TypeInfo(exprType, _), exprRange) as thisArg) ->
            ...
            | GenericParam(name = name) ->
                genericTypeInfoError name |> addError com ctx.InlinePath exprRange
            | _ -> ()
            ...
            | "get_Namespace" ->
                getTypeFullName false exprType |> splitFullName |> fst |> returnString r
            | "get_IsArray" ->
                match exprType with
                | Array _ -> true
                | _ -> false
...
let tryCall
    ...
    (args: Expr list)
    =
    // stdout.WriteLine $"call: {info.DeclaringEntityFullName} : {info.CompiledName} {args}"
    ...
    | "System.Reflection.MemberInfo" ->
        ...
        | Some c, "GetFields" ->
            ...
            |> Some
        | Some c, "GetValue" ->
            Helper.LibCall(com, "Reflection", "getValue", t, c :: args, ?loc = r) |> Some
        | Some c, "get_Name" ->
            match c with
...
let tryType typ =
    ...
    | Builtin kind ->
        ...
        | FSharpChoice genArgs ->
            Some($"{Types.choiceNonGeneric}`{List.length genArgs}", results, genArgs)
